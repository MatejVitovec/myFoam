...
{
    defineTypeNameAndDebug(relaxationLoss, 0);
    addToRunTimeSelectionTable(functionObject, relaxationLoss, dictionary);
}
}
...
Foam::functionObjects::relaxationLoss::relaxationLoss
(
    const word& name,
    const Time& runTime,
...
bool Foam::functionObjects::relaxationLoss::read(const dictionary& dict)
{
    return fvMeshFunctionObject::read(dict);
}


bool Foam::functionObjects::relaxationLoss::execute()
{
    return true;
}


bool Foam::functionObjects::relaxationLoss::write()
{
    ...
    dimensionedScalar totalLoss = fvc::domainIntegrate(zRelax);

    Log << "    Relaxation losses field " << zRelax.name()
        << " to " << time_.timeName() << ", total losses: " << totalLoss.value() << endl;
