...
// dynamicCode:
// SHA1 = 735b830ab3c0977bd06709c196fc267ad2d082b6
//
// unique function name that can be checked if the correct library version
// has been loaded
extern "C" void switchAlphaInlet_735b830ab3c0977bd06709c196fc267ad2d082b6(bool load)
{
    if (load)
    {
...
//{{{ begin code
    #line 36 "/home/matejv/myFoam/tutorials/SE1050BL3/0/alpha/boundaryField/inlet"
const scalar lowValue = 1e-10;
            const scalar highValue = 1e-5;
            const label switchIterStart = 7000;
            const label switchIterEnd = 11000;

            label iter = this->db().time().timeIndex();
            
            scalar val = lowValue;

            if(iter > switchIterEnd)
            {
                val = highValue;
            }
            if(iter >= switchIterStart && iter <= switchIterEnd)
            {
                const scalar psi = (scalar(iter) - switchIterStart)/(switchIterEnd - switchIterStart);
                const scalar G = -sqr(psi)*(2.0*psi - 3.0);
                val = G*highValue + (1.0 - G)*lowValue;
                //Info << "psi: " << psi << " G: " << G << " val: " << val << endl;
            }

            Info << "inlet value: " << val << endl;
